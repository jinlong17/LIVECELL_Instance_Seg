...
class MaxViT_MS_DCT(nn.Module):
    ...
    def __init__(
        ...
        """ Constructor method """
        # Call super constructor
        super(MaxViT_MS_DCT, self).__init__()
        # Check parameters
        ...
        # window_size = [tuple([16,16]), tuple([8,8]),tuple([4,4]),tuple([2,2])]####TODO:jinlong
        ...
        for index, (depth, channel) in enumerate(zip(depths, channels)):
            ...
            stages.append(
                MaxViTStage(
                    ...
                    num_heads=num_heads,
                    # grid_window_size=grid_window_size,
                    grid_window_size=window_size[index],####TODO:jinlong
                    attn_drop=attn_drop,
                    drop=drop,
    ...
    def DCT_process(self,x: torch.Tensor):

        # init DCT matrix
        size=640
        device = x.device
        # self._DCT_all = nn.Parameter(torch.tensor(DCT_mat(size)).float(), requires_grad=False).to(device)
        # self._DCT_all_T = nn.Parameter(torch.transpose(torch.tensor(DCT_mat(size)).float(), 0, 1), requires_grad=False).to(device)
        ...
        low_filter = Filter(size, 0, size // 16).cuda()
        middle_filter = Filter(size, size // 16, size // 8).cuda()
        high_filter = Filter(size, size // 8, size).cuda()
        all_filter = Filter(size, 0, size * 2).cuda()
    ...
    def feature_dct_15(self, input: torch.Tensor) -> torch.Tensor:
        ...
        # dct_f_list, _= torch.max(dct_f_list, dim=1, keepdim=True)
    ...
    def feature_dct_multi_fusion(self, input: torch.Tensor) -> torch.Tensor:
        ...
        dct_f_list = torch.cat(dct_f_list, dim=1)
        dct_f_list = torch.max(dct_f_list, dim=1, keepdim=True)
        combine = torch.cat([rgb_f, dct_f_list], dim=1)
